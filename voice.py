...
fileHandler = logging.FileHandler('voice.log', mode='w', encoding='UTF-8')
...
def message_handler(message):
    ...
    for i in range(len(alerts)):
        ...
        severity = eval(str(labels))["severity"]
        description = eval(str(annotations))["description"]
        message = "{},状态为{}".format(description, status)
...
# 发送到阿里云语音服务的函数
def send_voice(callednumber, voicemessage):
    client = AcsClient('xxxxxx', 'xxxxxx')
    ...
    request2.set_TtsCode("TTS_xxxxx")
    # request2.set_TtsParam("{\"name\":\"测试\",\"time\":\"2019年\"}")
    params = str({"content": voicemessage})
...
class voice(object):
    def on_post(self, req, resp):
        ...
        try:
            ...
            numbers = ['xxxxxx', 'xxxxxx']
            for i in range(len(messages)):
                    send_voice(numbers[0], str(messages[i]))
                    send_voice(numbers[1], str(messages[i]))
...
app = falcon.API()
voice = voice()
app.add_route('/voice', voice)


# pip install aliyun-python-sdk-dyvmsapi
# pip install requests falcon waitress
# 启动 nohup waitress-serve --port=23333 voice:app > alertmanager_wechatrobot.log &
